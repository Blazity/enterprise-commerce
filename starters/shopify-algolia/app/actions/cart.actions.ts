...
export async function addCartItem(
	...
	if (isDemoMode()) {
		return {
			ok: false,
			message:
				"Demo mode active. Filtering, searching, and adding to cart disabled.",
		};
	}
	...
	const availability = await getItemAvailability({
		cartId,
		variantId: makeShopifyId(variantId, "ProductVariant"),
		productId: makeShopifyId(productId, "Product"),
	});
	if (
		!availability ||
		availability.inCartQuantity >= availability.inStockQuantity
	) {
		return {
			ok: false,
	...
	console.log({ cartId, variantId, productId });
