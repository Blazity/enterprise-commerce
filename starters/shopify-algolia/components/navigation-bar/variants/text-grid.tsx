        ...
        <div className="submenu__inner flex w-full flex-col gap-4" key={singleCategory.text}>
          {singleCategory.href ? (
            <Link href={`${isAi ? "/ai" : ""}${singleCategory.href}`} prefetch={false}>
              <h4 className="submenu__title text-[22px] font-semibold underline-offset-4 hover:underline">{singleCategory.text}</h4>
            </Link>
          ) : (
          ...
          <ul className="submenu__list flex flex-col items-start justify-start gap-2 text-left">
            {singleCategory?.items?.map((item) => (
              <li key={item.text}>
                <Link href={`${isAi ? "/ai" : ""}${item.href}`} prefetch={false} className=" underline-offset-4 hover:underline">
                  {item.text}
                </Link>
              </li>
            ))}
