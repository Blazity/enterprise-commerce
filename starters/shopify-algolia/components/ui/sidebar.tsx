...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn("group sticky top-0 flex h-screen text-sidebar-foreground md:block", variant === "floating" && "p-2", className)}
      data-state={state}
    ...
    >
      <div
        data-sidebar="sidebar"
        className={cn(
          "flex h-full flex-col overflow-auto bg-sidebar transition-[width] duration-200",
          {
            "w-[--sidebar-width]": state === "expanded",
            "w-0 overflow-hidden": collapsible === "offcanvas" && state === "collapsed",
      ...
      {(collapsible === "offcanvas" || collapsible === "icon") && <SidebarRail className="hidden md:flex" />}
    </div>
  )
})
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button">>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-1 transition-all ease-linear hover:bg-sidebar-border group-data-[side=left]:right-0 group-data-[side=right]:left-0 sm:block",
