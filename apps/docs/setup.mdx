...
Remember to generate your own `FLAGS_SECRET` using `node -e "console.log(crypto.randomBytes(32).toString('base64url'))"`.
...
- Locate and copy the `Master key`. This key should be securely stored in your `.env.local` file under the `MEILISEARCH_MASTER_KEY` variable.
...
- Go to **Attributes** section, and:
  ...
  - Ensure **Filterable attributes** are set to `["collections","collections.title","flatOptions","handle","minPrice","tags","variants.availableForSale","vendor"]`
  - Ensure **Sortable attributes** are set to `["minPrice","updatedAtTimestamp"]`
