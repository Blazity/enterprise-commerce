...
export type SingleAdminProductQuery = { product?: AdminTypes.Maybe<(
    Pick<AdminTypes.Product, 'id' | 'handle' | 'title' | 'description' | 'descriptionHtml' | 'vendor' | 'tags' | 'updatedAt' | 'createdAt'>
    & { options: Array<Pick<AdminTypes.ProductOption, 'id' | 'name' | 'values'>>, collections: { nodes: Array<(
        Pick<AdminTypes.Collection, 'handle' | 'title' | 'descriptionHtml' | 'id' | 'description' | 'updatedAt'>
        & { image?: AdminTypes.Maybe<Pick<AdminTypes.Image, 'url' | 'altText' | 'width' | 'height'>>, seo: Pick<AdminTypes.Seo, 'title' | 'description'> }
      )> }, priceRange: { maxVariantPrice: Pick<AdminTypes.MoneyV2, 'amount' | 'currencyCode'>, minVariantPrice: Pick<AdminTypes.MoneyV2, 'amount' | 'currencyCode'> }, variants: { edges: Array<{ node: (
